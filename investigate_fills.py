...
def get_trades(client: KalshiClient, ticker: str, limit: int = 100) -> list[dict]:
    trades = []
    cursor = None
    while True:
        ...
        body   = client._get("/markets/trades", params)
...
def get_my_orders(client: KalshiClient, ticker: str) -> list[dict]:
    body = client._get("/portfolio/orders", {"ticker": ticker, "limit": 100})
    return body.get("orders", [])


def parse_ts(ts: str) -> datetime:
    # Kalshi uses RFC3339 / ISO8601
...
def main():
    ...
    my_order_times = {}
    try:
        my_orders = get_my_orders(client, ticker)
        if my_orders:
    ...
    # ── All trades ────────────────────────────────────────────────────────────
    trades = get_trades(client, ticker, args.limit)
    if not trades:
        print("\nNo trades found.")
        return
